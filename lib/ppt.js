...
function Stage(el, host){
    this.el = el
    this.host = host
    this.animate = this.el.getAttribute('animate') || 'slideUp'
    this.init()
}
...
Stage.prototype = {
    ...
    init: function(){
        ...
        this.blocs = [],this.currentBloc = 0

        toArray(rawBlocs).map(function(raw){
            var animate = raw.getAttribute('animate') || 'slideUp'
