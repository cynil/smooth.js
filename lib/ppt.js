...
PPT.prototype = {
        ...
        hm(document.body).on('tap', function(event){
            ...
            if(current){
                anime($(current.el), current.animate, function(){
                    ...
                    console.log('current: stages[' + self.stages.indexOf(self.currentStage) + '], blocs[' + self.currentStage.currentBloc + ']')
    ...
    anchor: function(tag, type, target){
        ...
        if(targetStage === undefined) {
            throw new Error('smooth.js: anchor bind failed, target stage not found.')
        }

        hm(tag).on(type, function(event){
